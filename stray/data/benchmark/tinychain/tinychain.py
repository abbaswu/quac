...
from typing import (
 Iterable, NamedTuple, Dict, Mapping, Type, Union, get_type_hints, Tuple, List, MutableMapping,
    Callable, Any, Set, NoneType)
...
class Transaction(NamedTuple):
    ...
    # ~
    @classmethod # Edit: classmethod
    def create_coinbase(cls:Type['Transaction'], pay_to_addr:str, value:int, height:int)->'Transaction':
        return cls(
            txins=[TxIn(
                to_spend=None,
...
class Block(NamedTuple):
    ...
    # @property
    # def id(self): return sha256d(self.header())
...
active_chain: List[Block] = [genesis_block]

# Branches off of the main chain.
side_branches: List[List[Block]] = []
...
def locate_block(block_hash, chain=None): # ～
    global_chains = []
    for c in side_branches:
        global_chains.append(c)
    global_chains.append(active_chain) # Edit: star
    chains = [chain] if chain else global_chains
        ...
        for tx in block.txns:
            ...
            if not tx.is_coinbase():
                for txin in tx.txins:
                    ...
                    rm_from_utxo(txid, txout_idx) # Edit: star
    ...
    for tx in block.txns:
        ...
        for txin in tx.txins:
            if txin.to_spend:  # Account for degenerate coinbase txins.
                txout, tx, txout_idx, is_coinbase, height = find_txout_for_txin(txin, chain) # Edit: star
                add_to_utxo(txout, tx, txout_idx, is_coinbase, height)
...
def find_txout_for_txin(txin, chain): # ～
    txid, txout_idx = txin.to_spend # if to_speed None this will be an error # Edit: star

    for tx, block, height in txn_iterator(chain):
        if tx.id() == txid: # Edit: property
            txout = tx.txouts[txout_idx] # List may not be indexed
            return (txout, tx, txout_idx, tx.is_coinbase(), height) # Edit: property
...
# @with_lock(chain_lock)
# ～
def try_reorg(branch:List[Block], branch_idx:int, fork_idx:int)->bool: # Edit: nested
...
def add_to_utxo(txout, tx, idx, is_coinbase, height):
    utxo = UnspentTxOut(txout.value, txout.to_address, txid=tx.id(), txout_idx=idx, is_coinbase=is_coinbase, height=height)
...
# ----------------------------------------------------------------------------
# ～
def get_next_work_required(prev_block_hash):
    """
...
# ～
def calculate_fees(block: Block)->int: # Edit: nested
    """
...
# ～
def validate_signature_for_spend(txin, utxo, txn):
...
def validate_block(block):
    ...
    if block.timestamp - int(time.time()) > Params.MAX_FUTURE_BLOCK_TIME:
...
def select_from_mempool(block:Block)->Block:
    ...
    def check_block_size(b:Block) -> bool:
        return len(serialize(block)) < Params.MAX_BLOCK_SERIALIZED_SIZE

    def try_add_to_block(block:Block, txid:int) -> Block:
...
def get_merkle_root_of_txns(txns):
    return get_merkle_root([t.id for t in txns]) # Edit: star


@lru_cache(maxsize=1024)
def get_merkle_root(leaves:List[str])->MerkleNode: # Edit: nested
    """Builds a Merkle tree and returns the root given some leaf values."""
    if len(leaves) % 2 == 1:
        leaves = leaves + (leaves[-1],)

    def find_root(nodes:Iterable[MerkleNode])-> MerkleNode:
        newlevel = [
...
class GetBlocksMsg:  # Request blocks during initial sync
    """
...
class InvMsg:  # Convey blocks to a peer who is doing initial sync
    blocks: List[str]
    # ~
    def handle(self, sock, peer_hostname):
...
class GetUTXOsMsg:  # List all UTXOs
    def handle(self, sock, peer_hostname):
        sock.sendall(encode_socket_data(list(utxo_set.items())))


class GetMempoolMsg:  # List the mempool
    def handle(self, sock, peer_hostname):
        sock.sendall(encode_socket_data(list(mempool.keys())))


class GetActiveChainMsg:  # Get the active chain in its entirety.
    def handle(self, sock, peer_hostname):
...
# ~
def int_to_8bytes(a): return binascii.unhexlify(f"{a:0{8}x}")
...
# ~
def serialize(obj:object)->str: # Edit: nested
...
# ~
def deserialize(serialized: str) -> object: # Edit: nested
...
# # Main
# # ----------------------------------------------------------------------------
# Edit: add int conversion to forbid type error
