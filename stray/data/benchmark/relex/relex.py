...
class RelationInstancesReader(DatasetReader):
    ...
    # Edit: rels should be list[str] since it's the argument of MultiLabelField(rels), which only takes sequences. 
...
class MultilabelAveragePrecision(Metric):
    ...
    def _thresholded_average_precision_score(self, precision, recall):
        if len(precision) == 0:
            return 0, -1
        # Edit: convert to numpy function to make it static typable
        index = np.argmin(np.abs(recall - np.int(self.recall_thr)))
        
        filtered_precision = precision[:int(index) + 1]
        filtered_recall = recall[:int(index) + 1]
