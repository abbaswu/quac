...
import typing
from typing import Dict, Callable, Match, Union

import data.benchmark.pendulum as pendulum
from data.benchmark.pendulum.datetime import DateTime

from data.benchmark.pendulum.locales.locale import Locale
from data.benchmark.pendulum.mixins.default import FormattableMixin
...
class Formatter:
    ...
    _TOKENS_RULES:Dict[str, Callable] = {
        # Year
    ...
    _DEFAULT_DATE_FORMATS:Dict[str,str] = {
        "LTS": "h:mm:ss A",
        "LT": "h:mm A",
        "L": "MM/DD/YYYY",
        "LL": "MMMM D, YYYY",
    ...
    # Edit: dict
    def format(
        self, dt:FormattableMixin, fmt:str, locale:Union[Locale, str]=None
    )->str:  
        """
    ...
    def _format_token(
        self, dt:DateTime, token:str, locale:Locale
    )->str:  
        """
    ...
    def _format_localizable_token(
        self, dt:DateTime, token:str, locale:Union[str, Locale]
    )->str:  
        """
    ...
    def _get_parsed_values(
        self, m:Match, parsed:Dict, locale:Union[str,Locale], now:DateTime
    ):  
    ...
    def _get_parsed_value(
        self, token:str, value:int, parsed:Dict, now:DateTime
    )->None:  
    ...
    def _get_parsed_locale_value(
        self, token:str, value:int, parsed:Dict, locale:Locale
    )->None:  
        if token == "MMMM":
            unit = "month"
            match = "months.wide"
