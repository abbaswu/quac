...
from typing import (
 Iterable, NamedTuple, Dict, Mapping, Union, get_type_hints, Tuple, List, MutableMapping,
    Callable, Any, Set)
...
def connect_block(block,
    ...
    if chain_idx == ACTIVE_CHAIN_IDX:
        for tx in block.txns:
            ...
            #         rm_from_utxo(*txin.to_spend) # not yet supported
...
# @with_lock(chain_lock)
# ～
def try_reorg(branch:List[Block], branch_idx:int, fork_idx:int)->bool:
...
# ～
def calculate_fees(block: Block)->int:
    """
...
# ~
def build_spend_message(to_spend, pk, sequence, txouts:Any):
...
def get_merkle_root(*leaves:MerkleNode)->MerkleNode:
...
peer_hostnames:Set[str] = {p for p in os.environ.get('TC_PEERS', '').split(',') if p}
...
WALLET_PATH:str = os.environ.get('TC_WALLET_PATH', 'wallet.dat')
