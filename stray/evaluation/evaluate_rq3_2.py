...
def read_unittests(ot, ou):
    for f in glob(r'result/data-benchmark-unittests_typpete-*'):
        with open(f) as f:
            for l in f:
                l.split(';')
...
for i, proj in enumerate(projects):
    ...
    ou = set()
    if proj == 'scion':
        f = 'result/data-benchmark-scion-lib-packet-scion_addr.py_whole_'
        with open(f) as f:
            for l in f:
                l.split(';')
                name = l.split(';')[0].strip()
        ...
        read_unittests(ot, ou)
    else:
        f = f'result/data-benchmark-unittests-{proj}.py_whole_'
        with open(f) as f:
            for l in f:
                l.split(';')
                name = l.split(';')[0].strip()
