...
from extyper.types import (
    MaybeTypes, Type, AnyType, NoneType, TypeVisitor, Instance, UnboundType, TypeVarType, CallableType,
...
from extyper.nodes import INVARIANT, COVARIANT, CONTRAVARIANT
...
def join_simple(declaration: Optional[Type], s: Type, t: Type) -> ProperType:
    ...
    s = get_proper_type(s)
    t = get_proper_type(t)
    if isinstance(s, (list, set)) and isinstance(t, (list, set)):
        unioned = set(s)
        unioned.update(t)
        return unioned
    if (s.can_be_true, s.can_be_false) != (t.can_be_true, t.can_be_false):
        # if types are restricted in different ways, use the more general versions
        s = extyper.typeops.true_or_false(s)
        t = extyper.typeops.true_or_false(t)

    if isinstance(s, MaybeTypes) and isinstance(t, MaybeTypes):
        unioned = s.items+t.items
        return MaybeTypes.make_union(unioned)

        unioned = mypy.typeops.make_simplified_union(s.items+t.items)
        if isinstance(unioned, UnionType):
            return MaybeTypes.make_union(unioned)
        else:
            return MaybeTypes.make_union([unioned])
    ...
    if isinstance(declaration, UnionType):
        return extyper.typeops.make_simplified_union([s, t])
...
def join_types(s: Type, t: Type, instance_joiner: Optional[InstanceJoiner] = None) -> ProperType:
    ...
    """
    if extyper.typeops.is_recursive_pair(s, t):
...
class TypeJoinVisitor(TypeVisitor[ProperType]):
    ...
    def visit_union_type(self, t: UnionType) -> ProperType:
        if is_proper_subtype(self.s, t):
            ...
            return extyper.typeops.make_simplified_union([self.s, t])
    def visit_maybe_type(self, t: MaybeTypes) -> ProperType:
        if is_proper_subtype(self.s, t):
            return t
        else:

            s = self.s.items if isinstance(self.s, MaybeTypes) else [self.s]
            return MaybeTypes.make_union(s+t.items)
    ...
    def visit_tuple_type(self, t: TupleType) -> ProperType:
        ...
        if isinstance(self.s, TupleType) and self.s.length() == t.length():
            if self.instance_joiner is None:
                self.instance_joiner = InstanceJoiner()
            fallback = self.instance_joiner.join_instances(extyper.typeops.tuple_fallback(self.s),
                                                       extyper.typeops.tuple_fallback(t))
            ...
            return join_types(self.s, extyper.typeops.tuple_fallback(t))
    ...
    def default(self, typ: Type) -> ProperType:
        ...
        if isinstance(typ, Instance):
            ...
            return self.default(extyper.typeops.tuple_fallback(typ))
