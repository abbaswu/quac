...
from extyper.typeops import tuple_fallback, make_simplified_union
...
def is_same_type(left: Type, right: Type) -> bool:
    ...
    if isinstance(left, (list,set)) and isinstance(right, (list, set)):
        return all(is_same_type(x,y) for x, y in zip(left, right))
...
class SameTypeVisitor(TypeVisitor[bool]):
    ...
    def __init__(self, right: ProperType) -> None:
        # if isinstance(right, TupleType) and right.partial_fallback is not None:
        #     # named tuple
        #     self.right = right.partial_fallback
        # else:
            self.right = right
    ...
    def visit_instance(self, left: Instance) -> bool:
        if isinstance(self.right, TupleType) and self.right.partial_fallback is not None:
            # named tuple
            self.right = self.right.partial_fallback
    ...
    def visit_literal_type(self, left: LiteralType) -> bool:
        ...
        if isinstance(self.right, Instance):
            return is_same_type(left.fallback, self.right)
        else:
            return False
