...
from extyper.types import NoneType, TypeVisitor
import extyper.types as types
...
class TypeIndirectionVisitor(TypeVisitor[Set[str]]):
    ...
    def visit_maybe_type(self, t: types.MaybeTypes) -> Set[str]:
        return self._visit(t.items)
    ...
    def visit_type_type(self, t: types.TypeType) -> Set[str]:
        if isinstance(t.item, NoneType) or t.item is None:
            return set()
