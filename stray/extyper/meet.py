...
from extyper.types import (
    MaybeTypes, Type, AnyType, TypeVisitor, UnboundType, NoneType, TypeVarType, Instance, CallableType,
    TupleType, TypedDictType, ErasedType, UnionType, PartialType, DeletedType,
    UninhabitedType, TypeType, TypeOfAny, Overloaded, FunctionLike, LiteralType,
    ProperType, get_proper_type, get_proper_types, TypeAliasType, TypeGuardedType
)
from extyper.subtypes import is_equivalent, is_subtype, is_callable_compatible, is_proper_subtype
from extyper.erasetype import erase_type
from extyper.maptype import map_instance_to_supertype
from extyper.typeops import tuple_fallback, make_simplified_union, is_recursive_pair
...
def meet_types(s: Type, t: Type) -> ProperType:
    ...
    if isinstance(s, UnionType) and not isinstance(t, UnionType):
        s, t = t, s
    ret = t.accept(TypeMeetVisitor(s))
    ret = t.accept(TypeMeetVisitor(s))
...
def narrow_declared_type(declared: Type, narrowed: Type) -> Type:
    ...
    elif isinstance(declared, MaybeTypes):
        return declared
...
def is_overlapping_types(left: Type,
    ...
    illegal_types = (UnboundType, ErasedType, DeletedType, PartialType)
...
class TypeMeetVisitor(TypeVisitor[ProperType]):
    ...
    def visit_maybe_type(self, t: MaybeTypes) -> ProperType:
        if isinstance(self.s, MaybeTypes):
            meets: List[Type] = []
            for x in t.items:
            ...
            meets = [meet_types(x, self.s)
                     for x in t.items]
        return MaybeTypes.make_union(meets)
