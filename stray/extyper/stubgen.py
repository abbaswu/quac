...
import extyper.parse
import extyper.traverser
import extyper.mixedtraverser
import extyper.util
from extyper import defaults
...
from extyper.stubgenc import generate_stub_for_c_module
from extyper.stubutil import (
...
from extyper.stubdoc import parse_all_signatures, find_unique_signatures, Sig
from extyper.options import Options as MypyOptions
...
from extyper.build import build
from extyper.errors import CompileError, Errors
from extyper.traverser import all_yield_expressions, has_return_statement, has_yield_expression
from extyper.moduleinspect import ModuleInspect
...
class DefinitionFinder(extyper.traverser.TraverserVisitor):
...
class ReferenceFinder(extyper.mixedtraverser.MixedTraverserVisitor):
...
class StubGenerator(extyper.traverser.TraverserVisitor):
    ...
    def visit_import_from(self, o: ImportFrom) -> None:
        ...
        if self.module:
            full_module, ok = extyper.util.correct_relative_import(
...
class SelfTraverser(extyper.traverser.TraverserVisitor):
    def __init__(self) -> None:
...
def find_module_paths_using_search(modules: List[str], packages: List[str],
    ...
    typeshed_path = default_lib_path(extyper.build.default_data_dir(), pyversion, None)
...
def parse_source_file(mod: StubSource, mypy_options: MypyOptions) -> None:
    ...
    with open(mod.path, 'rb') as f:
        data = f.read()
    source = extyper.util.decode_python_encoding(data, mypy_options.python_version)
    errors = Errors()
    mod.ast = extyper.parse.parse(source, fnam=mod.path, module=mod.module,
...
def generate_stubs_with_modules(options: Options, graph) -> None:
    """Main entry point for the program."""
    mypy_opts = mypy_options(options)
    path2ast = {x.abspath:x.tree for x in graph.values()}
    ...
    for mod in py_modules:
        assert mod.path is not None, "Not found module was not skipped"
        if mod.path in path2ast:
            mod.ast = path2ast[mod.path]
...
def main_args(args, graph):
    options = parse_options(args[1:])
    generate_stubs_with_modules(options, graph)

def main() -> None:
    extyper.util.check_python_version('stubgen')
