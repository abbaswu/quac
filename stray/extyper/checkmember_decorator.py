...
from extyper.types import (
    ...
    DeletedType, NoneType, TypeType, has_type_vars, get_proper_type, ProperType, MaybeTypes
)
from extyper.nodes import (
    MemberExpr, OpExpr, TypeInfo, FuncBase, Var, FuncDef, SymbolNode, SymbolTable, Context,
...
from extyper.expandtype import expand_type_by_instance, freshen_function_type_vars
from extyper.erasetype import erase_typevars
from extyper.plugin import AttributeContext
from extyper.typeanal import set_any_tvars
from extyper import message_registry
from extyper import subtypes
from extyper import meet
...
class MemberContext:
    ...
    def __init__(self,
                 ...
                 module_symbol_table: Optional[SymbolTable] = None, object_node=None) -> None:
    ...
    def copy_modified(self, *, messages: Optional[MessageBuilder] = None,
        ...
        mx = MemberContext(self.is_lvalue, self.is_super, self.is_operator,
                           ...
                           self.self_type, self.module_symbol_table, object_node = self.object_node)
...
def analyze_member_access(name: str,
                          ...
                          module_symbol_table: Optional[SymbolTable] = None, object_node = None, member_expr = None) -> Type:
    ...
    mx.object_node = object_node
    mx.member_expr = member_expr
...
def _analyze_member_access(name: str,
    ...
    elif isinstance(typ, MaybeTypes):
        return analyze_soft_member_access(name, typ, mx)
...
def convert_overload(method, name, mx, typ):
    if isinstance(method.type, Overloaded):
        ...
        for item in items:
            ...
            new_items.append(member_type)
        new_type = Overloaded(new_items)
        # method.type = new_type 这里不能覆盖，覆盖掉之后后面再bindself，就会出问题
        return new_type
...
def analyze_instance_member_access(name: str,
    ...
    if method is not None and method.line != -1 and method.type is None:
        if method in mx.chk.method_typing:
            ...
            for globals_ in annos:
                # for global_ in globals_:
                #     mx.chk.store_type_local(global_[0], global_[1])
                node_type = annos[globals_]
                # should be an overloaded callableType
                assert isinstance(node_type, Overloaded)
                method.type = node_type
                node_type = convert_overload(method, name, mx, typ)
                method.type = None
                mx.chk.expr_checker.add_infer_type((mx.member_expr, node_type), globals_)
                member_type.append(node_type)
            member_type = MaybeTypes.make_union(member_type)
            return member_type
        else:
            # return AnyType(TypeOfAny.from_error)
    ...
    if method:
        if isinstance(method.type, Overloaded):
            ...
            for item in items:
                ...
                new_items.append(member_type)
            new_type = Overloaded(new_items)
            method.type = origin_type
            # method.type = new_type 这里不能覆盖，覆盖掉之后后面再bindself，就会出问题
            return new_type
        elif not isinstance(method, Decorator):
            if method.is_property:
                assert isinstance(method, OverloadedFuncDef)
                first_item = cast(Decorator, method.items[0])
                return analyze_var(name, first_item.var, typ, info, mx)
...
def analyze_union_member_access(name: str, typ: UnionType, mx: MemberContext) -> Type:
    ...
    for subtype in typ.relevant_items():
        if not isinstance(subtype, NoneType):
    ...
    if any([isinstance(x, AnyType) and x.type_of_any == 5 for x in results]):
        if hasattr(mx, 'object_node') and mx.object_node is not None:
            mx.chk.expr_checker.add_single_incompatible(mx.object_node, typ)
    mx.msg.disable_type_names -= 1
    return make_simplified_union(results)
def analyze_soft_member_access(name: str, typ: MaybeTypes, mx: MemberContext) -> Type:
    mx.msg.disable_type_names += 1
    results = []
    for subtype in typ.relevant_items():
        # Self types should be bound to every individual item of a union.
        item_mx = mx.copy_modified(self_type=subtype)
        ret_typ = _analyze_member_access(name, subtype, item_mx)
        if not isinstance(ret_typ, AnyType):
            results.append(ret_typ)
    mx.msg.disable_type_names -= 1
    return MaybeTypes.make_union(results)
...
def analyze_member_var_access(name: str,
    ...
    if mx.is_super:
        ...
        if mx.chk and mx.chk.should_suppress_optional_error([itype]):
            return AnyType(TypeOfAny.from_error)
        if hasattr(mx, 'object_node') and mx.object_node is not None:
            mx.chk.expr_checker.add_single_incompatible(mx.object_node, itype)
        else:
            pass
...
def analyze_descriptor_access(instance_type: Type,
                              ...
                              msg: MessageBuilder,
                              context: Context, *,
                              chk: 'extyper.checker.TypeChecker') -> Type:
...
def analyze_var(name: str,
    ...
    if typ is None and var in mx.chk.var_node:
        ...
        # assert len(node) == 1
        node = node[0]
        node_type = mx.chk.type_map[node]

        # mx.chk.store_type_local(node, node_type)
        result = node_type
        if hasattr(mx, "member_expr"):
            if isinstance(node_type, MaybeTypes):
                node_types = node_type.items
            else:
                node_types = [node_type]
            for node_type in node_types:
                mx.chk.expr_checker.add_infer_type((mx.member_expr, node_type), [(node, node_type)])
        return result
    if typ:
