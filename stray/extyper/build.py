...
from extyper.types import AnyType, NoneType, TypeOfAny, Instance, CallableType, TypeType, TypeVarType
from extyper.nodes import ClassDef, FuncDef, MypyFile, ImportBase, Import, ImportFrom, ImportAll, SymbolTable
from extyper import nodes
from extyper.semanal_pass1 import SemanticAnalyzerPreAnalysis
...
if TYPE_CHECKING:
    from extyper.report import Reports  # Avoid unconditional slow import
from extyper.fixup import fixup_module
from extyper.modulefinder import (
    BuildSource, compute_search_paths, FindModuleCache, SearchPaths, ModuleSearchResult,
    ModuleNotFoundReason
)
from extyper.nodes import Expression, TypeInfo
from extyper.options import Options
from extyper.parse import parse
from extyper.stats import dump_type_stats
from extyper.types import Type
from extyper.version import __version__
from extyper.plugin import Plugin, ChainedPlugin, ReportConfigContext
from extyper.plugins.default import DefaultPlugin
from extyper.fscache import FileSystemCache
from extyper.metastore import MetadataStore, FilesystemMetadataStore, SqliteMetadataStore
from extyper.typestate import TypeState, reset_global_state
from extyper.renaming import VariableRenameVisitor
from extyper.config_parser import parse_mypy_comments
from extyper.freetree import free_tree
from extyper.stubinfo import legacy_bundled_packages, is_legacy_bundled_package
...
def build(sources: List[BuildSource],
    ...
    try:
        result = _build(
            sources, options, alt_lib_path, flush_errors, fscache, stdout, stderr, extra_plugins,modules=modules
        )
...
def _build(sources: List[BuildSource],
    ...
    if options.report_dirs:
        ...
        from extyper.report import Reports  # noqa
    ...
    manager = BuildManager(data_dir, search_paths,
                           ...
                           fscache=fscache,
                           stdout=stdout,
                           stderr=stderr, user_modules=modules)
    ...
    reset_global_state()
    try:
        graph = dispatch(sources, manager, stdout, t0)
...
class BuildManager:
    ...
    def __init__(self, data_dir: str,
        ...
        self.union_errors = {}
        self.incompatible = {}
        self.single_incompatible = {}
        self.added_attr = {}
        self.func_name_recorder = {}
        self.func_name_order = []
        ...
        self.func_s3 = defaultdict(dict)
        self.func_must = defaultdict(dict)
        self.func_candidates = defaultdict(dict)
        self.func_class = {}
        self.must_analyze = {}
        self.func_finished = defaultdict(dict)
        with open(f'results/funcs_ret-{self.options.tag}', 'w+') as f:
            pass
        with open(f'results/funcs-{self.options.tag}', 'w+') as f:
            pass
        with open(f'results/funcs_unfinished-{self.options.tag}','w+') as f:
            pass

        self.lexical_stat = defaultdict(set)
    ...
    def log_fine_grained(self, *message: str) -> None:
        ...
        elif extyper.build.DEBUG_FINE_GRAINED:
...
def invert_deps(deps: Dict[str, Set[str]],
    ...
    from extyper.server.target import trigger_to_target
...
def generate_deps_for_cache(manager: BuildManager,
    ...
    fake module FAKE_ROOT_MODULE if none are.
    """
    from extyper.server.deps import merge_dependencies  # Lazy import to speed up startup
...
class State:
    ...
    def type_suggest_init_pass(self, s1, s2, s3, user_defined, hierarchy_children, mode, func_name_reocrder=None) -> None:
        if self.options.semantic_analysis_only:
            return
        with self.wrap_context():
    ...
    def compute_fine_grained_deps(self) -> Dict[str, Set[str]]:
        ...
        if self.id in ('builtins', 'typing', 'types', 'sys', '_typeshed'):
            ...
            return {}
        from extyper.server.deps import get_dependencies  # Lazy import to speed up startup
...
def find_module_and_diagnose(manager: BuildManager,
    ...
    if isinstance(result, str):
        ...
        if (root_source  # Honor top-level modules
                ...
                or id in extyper.semanal_main.core_modules):  # core is always normal
...
def dispatch(sources: List[BuildSource],
    ...
    with open(f'results/funcs_time-{manager.options.tag}', 'w+') as f:
...
def merge_project(graph, sccs):
    new_sccs = []
    proj = set()
    for ascc in sccs:
        if any(not graph[node].ignore_all for node in ascc):
            proj.update(ascc)
        else:
            new_sccs.append(ascc)
    new_sccs.append(proj)
...
def process_graph(graph: Graph, manager: BuildManager) -> None:
    ...
    if 'bellman_ford' not in graph:
        sccs = merge_project(graph, sccs)
    start = time.time()
    for ascc in tqdm(sccs):
    ...
    manager.func_name_recorder = {x:y for x,y in manager.func_name_recorder.items() if x in manager.func_name_order}
    ordered_strs = sorted(manager.func_name_recorder.items(), key = lambda x:manager.func_name_order.index(x[0]))
    with open(f'results/funcs_res-{manager.options.tag}', 'w+') as f:
        for func in ordered_strs:
            f.write('\n'.join(func[1])+'\n')
    with open(f'results/lexical_stat-{manager.options.tag}', 'w+') as f:
        for const in manager.lexical_stat:
            f.write(str(const) + ':' + str(len(manager.lexical_stat[const])) + '\n')
    if sccs_left:
...
def process_project(graph: Graph, scc: List[str], manager: BuildManager) -> None:
    mode = 'PTI' # TTI CPA PTI
    stale = scc
    t1 = time.time()
    for id in stale:
        graph[id].parse_file()
    ...
    extyper.semanal_main.semantic_analysis_for_scc(graph, scc, manager.errors)

    user_type_s2 = []
    user_type_s1 = []
    user_type_s3 = []
    packages = set()
    with open('data/statistics/third-party-types.json') as f:
        third_party_types = json.load(f)

    for file_ in stale:
        for i in graph[file_].tree.imports:
            if isinstance(i, Import):
                for id_ in i.ids:
                    packages.add(id_[0].split('.')[0])
            else:
                packages.add(i.id.split('.')[0])
    for p in packages:
    ...
    user_type_s2.append(float_instance)

    # if mode != 'PTI':
    user_type_s2.append(bool_instance)
    user_type_s2.append(int_instance)
    # hierarchy_children[float_instance] = [int_instance]
    # hierarchy_children[int_instance] = [bool_instance]
    ...
    # oracle = ['Block', 'TxIn', 'TxOut', 'Transaction', 'UnspentTxOut']
    builtins = ['Set', 'List', 'Tuple', 'Dict', 'defaultdict', 'MutableMapping', 'Iterable', 'Mapping', 'NamedTuple', 'date', 'datetime']
    all_user_defined = set()
    for module in scc:
        ...
        if module in manager.user_modules:
            ...
            for name in state.tree.names:
                if name not in builtins:
                    ...
                    node = symbol.node
                    if isinstance(node, TypeInfo):
                        if len(node.mro) <= 2 or node.is_named_tuple:
                            any_type = AnyType(TypeOfAny.from_omitted_generics)
                            user_type = Instance(node, [any_type] * len(node.defn.type_vars))
                            user_type_s2.append(user_type)
                            user_defined.append(str(user_type))
                        # abc counts one level
                        if len(node.mro) <= 3 and any(m.fullname == 'abc.ABC' for m in node.mro):
                            any_type = AnyType(TypeOfAny.from_omitted_generics)
                            user_type = Instance(node, [any_type] * len(node.defn.type_vars))
                            user_type_s2.append(user_type)
                            user_defined.append(str(user_type))
                        # ust = set(node.mro).intersection(all_user_defined)
                        # if len(ust) <= 1:
                        #     any_type = AnyType(TypeOfAny.from_omitted_generics)
                        #     user_type = Instance(node, [any_type] * len(node.defn.type_vars))
                        #     user_type_s2.append(user_type)
                        #     user_defined.append(str(user_type))
                        # mixin counts one level
                        if any(m.fullname.lower().find('mixin') != -1 for m in node.mro):
                            any_type = AnyType(TypeOfAny.from_omitted_generics)
                            user_type = Instance(node, [any_type] * len(node.defn.type_vars))
                            user_type_s2.append(user_type)
                            user_defined.append(str(user_type))
    ...
    # manager.func_name_recorder.clear()
    t2 = time.time()
    with open(f'results/funcs_time-{manager.options.tag}', 'a+') as f:
    ...
    while unfinished_modules:
        ...
        previous_file = previous_file[-15:]
    ...
    print(list(set(previous_file))[0])
    try:
        with open(f'results/funcs_unfinished-{manager.options.tag}','a+') as f:
            f.write('\n'.join(set([x.node.fullname for y in previous for x in y]))+'\n')
    except Exception:
        pass
    for id in stale:
...
def process_stale_scc_check(graph: Graph, scc: List[str], manager: BuildManager) -> None:
    ...
    suggestions = [
        [# convolve ?
            [
                graph[stale[0]].type_checker().named_type('builtins.float')  
            ], 
            [
                graph[stale[0]].type_checker().named_type('builtins.float') 
            ], 
            [
                graph[stale[0]].type_checker().named_type('builtins.str'),
                graph[stale[0]].type_checker().named_type('builtins.bool')   
            ],
            [
                graph[stale[0]].type_checker().named_type('builtins.str'),
                graph[stale[0]].type_checker().named_type('numpy.ndarray')   
            ],
            [
                graph[stale[0]].type_checker().named_type('builtins.int'),
                graph[stale[0]].type_checker().named_type('builtins.bool')   
            ],
            [
                NoneType(0)
        ...
        [# parse rulestring
            ...
            [
                graph[stale[0]].type_checker().named_type_with_type_var('builtins.set', graph[stale[0]].type_checker().named_type('builtins.str'))            
            ]
        ...
        [# life rule
            ...
            [
                graph[stale[0]].type_checker().named_type('builtins.int'),
                graph[stale[0]].type_checker().named_type_with_type_var('builtins.list', graph[stale[0]].type_checker().named_type('builtins.int'))
            ],
            [
                NoneType(0),
                # graph[stale[0]].type_checker().named_type('torch.Tensor')
            ],
        ],
        [# conway classic
            [
        ...
        [# cell coverage
            [
                NoneType(0)
                # graph[stale[0]].type_checker().named_type_with_type_var('builtins.dict', graph[stale[0]].type_checker().named_type('builtins.str'), graph[stale[0]].type_checker().named_type('torch.Tensor'),),
            ],
            [
                graph[stale[0]].type_checker().named_type('numpy.ndarray'),
                NoneType(0),
        ...
        [# parse_plaintext
            [
                graph[stale[0]].type_checker().named_type('builtins.bytes'),
                graph[stale[0]].type_checker().named_type('builtins.str'),
            ],
            [
                NoneType(0),
        ...
        [# get_metadata
            [
                graph[stale[0]].type_checker().named_type_with_type_var('builtins.list', graph[stale[0]].type_checker().named_type('builtins.str'))
            ],
            [
                NoneType(0),
                graph[stale[0]].type_checker().named_dict_with_type_var('builtins.dict', graph[stale[0]].type_checker().named_type('builtins.str'), AnyType(0))
            ],
        ],
        [# load_file
            [
        ...
        [# parse_cell
            ...
            [
                NoneType(0),
                graph[stale[0]].type_checker().named_dict_with_type_var('builtins.dict', graph[stale[0]].type_checker().named_type('builtins.str'), graph[stale[0]].type_checker().named_type('builtins.str'),)
            ],
        ],
        [# cell_rle
            [
        ...
        [# parse_rle
            ...
            [
                NoneType(0),
                graph[stale[0]].type_checker().named_dict_with_type_var('builtins.dict', graph[stale[0]].type_checker().named_type('builtins.str'), graph[stale[0]].type_checker().named_type('builtins.int'),)
            ],
        ],
    ...
    def get_trivial_type(fdef) -> CallableType:
        ...
        return CallableType(
            ...
            manager.semantic_analyzer.builtin_type('builtins.function'))
    def check(a_):
        for id in stale:
            c = 0
            for i, d in enumerate(graph[id].tree.defs):
                if isinstance(d, FuncDef):
                    if d.type is None:
                        predictions = a_[c]
                        base = get_trivial_type(d)
                        now = base.copy_modified(arg_types=predictions[:-1], ret_type=predictions[-1])
                        d.type = now
            ...
            for i, d in enumerate(graph[id].tree.defs):
                if isinstance(d, ClassDef):
                    for dd in d.defs:
                        if isinstance(dd, FuncDef):
                            if dd.type is None:
                                predictions = a_[c]
                                base = get_trivial_type(dd)
                                now = base.copy_modified(arg_types=predictions[:-1], ret_type=predictions[-1])
                                dd.type = now
                            c += 1
            chk = graph[id].type_checker()
            chk.incompatible.clear()
            chk.single_incompatible.clear()
            chk.type_map.clear()
            chk.cache_type_map.clear()
            chk.var_node.clear()
            chk.err_cnts = 0
            graph[id].type_check_first_pass()
            type_errors = len(chk.incompatible) + len(chk.single_incompatible) + chk.err_cnts
            slots = [[isinstance(x, AnyType) for x in func] for func in a_]
            missing = sum([sum(func) for func in slots])
            all_ = sum([len(func) for func in slots])
            return missing + (all_ + 1) * type_errors

    def new_states(a_, suggestions):
        news = []
        for i, func in enumerate(suggestions):
            for j, preds in enumerate(func):
                if isinstance(a_[i][j], AnyType):
                    continue
                now_index = preds.index(a_[i][j])
                for k in range(now_index+1, len(preds)):
                    next_try = preds[k]
                    new = [[x for x in f] for f in a_]
                    new[i][j] = next_try
                    news.append(new)
                next_try = AnyType(0)
                new = [[x for x in f] for f in a_]
        ...
        news_tuple = [tuple([tuple(x) for x in new]) for new in news]
        for new in news_tuple:
            parent[new] = a_
        news_set = set(news_tuple)
        return news_set

    # set all function
    a = tuple([tuple([t[0] for t in func]) for func in suggestions]) # a list of function of slots
    score = check(a)
    min_score = score
    min_a = a
    parent = {}
    scores = {a:score}
    workset = new_states(a, suggestions)
    done = set()
    greedy = False
    while min_score > 0 and len(workset) > 0:
        now = workset.pop()
        score = check(now)
        score = check(now)
        scores[now] = score
        added_states = new_states(now, suggestions)
        done.add(now)
        if greedy and score < scores[parent[now]]:
            workset = added_states.difference(done)
        elif not greedy:
            workset = workset.union(added_states.difference(done))
        if score < min_score:
            min_score = score
            min_a = now
