...
from extyper.util import IdMapper
...
# semantic analyzer!
if TYPE_CHECKING:
    from extyper.type_visitor import (
...
class Type(extyper.nodes.Context):
...
class TypeAliasType(Type):
    ...
    def __init__(self, alias: Optional[extyper.nodes.TypeAlias], args: List[Type],
...
NOT_READY: Final = extyper.nodes.FakeInfo("De-serialization failure: TypeInfo not fixed")
...
class Instance(ProperType):
    ...
    def __init__(self, typ: extyper.nodes.TypeInfo, args: Sequence[Type],
...
class FunctionLike(ProperType):
    ...
    @abstractmethod
    def type_object(self) -> extyper.nodes.TypeInfo: pass
...
class CallableType(FunctionLike):
    ...
    def __init__(self,
        ...
        if isinstance(ret_type, (set, list)):
            ret_type = AnyType(0)
    ...
    def type_object(self) -> extyper.nodes.TypeInfo:
...
class MaybeTypes(ProperType):
    ...
    @staticmethod
    def make_union(items: Sequence[Type], line: int = -1, column: int = -1, from_cond=False) -> Type:
        items = list(set(items))
        if len(items) >= 1:
            return MaybeTypes(items, line, column)
        # elif len(items) == 1:
        #     return items[0]
        else:
            if from_cond:
    ...
    def accept(self, visitor: 'TypeVisitor[T]') -> T:
        return visitor.visit_maybe_type(self)
    ...
    def has_readable_member(self, name: str) -> bool:
        ...
        TODO: This should probably be refactored to go elsewhere.
        """
        return all((isinstance(x, MaybeTypes) and x.has_readable_member(name)) or
                   (isinstance(x, Instance) and x.type.has_readable_member(name))
                   for x in get_proper_types(self.relevant_items()))
    ...
    def deserialize(cls, data: JsonDict) -> 'UnionType':
        assert data['.class'] == 'UnionType'
        return MaybeTypes([deserialize_type(t) for t in data['items']])
...
class PartialType(ProperType):
    ...
    type: Optional[extyper.nodes.TypeInfo] = None
    var: extyper.nodes.Var
    ...
    def __init__(self,
                 type: 'Optional[extyper.nodes.TypeInfo]',
                 var: 'extyper.nodes.Var',
...
def get_proper_type(typ: Optional[Type]) -> Optional[ProperType]:
    ...
    """
    if isinstance(typ, set) or isinstance(typ, list):
        return typ
    if typ is None:
        return None
...
from extyper.type_visitor import (  # noqa
    TypeVisitor as TypeVisitor,
    SyntheticTypeVisitor as SyntheticTypeVisitor,
...
class TypeStrVisitor(SyntheticTypeVisitor[str]):
    ...
    def visit_maybe_type(self, t:MaybeTypes):
        s = self.list_str(t.items)
        return 'Maybe[{}]'.format(s)
...
def is_generic(tp: Type) -> bool:
    tp = get_proper_type(tp)
    if isinstance(tp, TypeVarType):
        return True
    if is_generic_instance(tp):
        return True
    if isinstance(tp, UnionType):
        return any(is_generic_instance(x) for x in tp.items)
    if isinstance(tp, TupleType):
        return any(is_generic(x) for x in tp.items)
