...
import extyper.modulefinder
import extyper.types
from extyper import nodes
from extyper.config_parser import parse_config_file
from extyper.options import Options
from extyper.util import FancyFormatter
...
def _verify_arg_default_value(
    ...
    if runtime_arg.default != inspect.Parameter.empty:
        if stub_arg.kind.is_required():
            ...
            if isinstance(stub_type, extyper.types.TypeVarType):
...
class Signature(Generic[T]):
    ...
    def from_overloadedfuncdef(stub: nodes.OverloadedFuncDef) -> "Signature[nodes.Argument]":
        ...
        def get_type(arg_name: str) -> extyper.types.ProperType:
            with extyper.state.strict_optional_set(True):
                all_types = [
                    arg.variable.type or arg.type_annotation for arg, _ in all_args[arg_name]
                ]
                return extyper.typeops.make_simplified_union([t for t in all_types if t])
...
def _verify_property(stub: nodes.Decorator, runtime: Any) -> Iterator[str]:
    ...
    runtime_type = get_mypy_type_of_runtime_value(runtime)
    func_type = (
        stub.func.type.ret_type if isinstance(stub.func.type, extyper.types.CallableType) else None
    )
    if (
        runtime_type is not None
...
def verify_typealias(
    ...
    if isinstance(runtime, Missing):
        ...
        return
    stub_target = extyper.types.get_proper_type(stub.target)
    if isinstance(stub_target, extyper.types.Instance):
        yield from verify(stub_target.type, runtime, object_path)
        return
    if isinstance(stub_target, extyper.types.UnionType):
        ...
        return
    if isinstance(stub_target, extyper.types.TupleType):
        ...
        return
    if isinstance(stub_target, extyper.types.AnyType):
        return
    yield Error(
...
def is_subtype_helper(left: extyper.types.Type, right: extyper.types.Type) -> bool:
    """Checks whether ``left`` is a subtype of ``right``."""
    left = extyper.types.get_proper_type(left)
    right = extyper.types.get_proper_type(right)
    if (
        isinstance(left, extyper.types.LiteralType)
        and isinstance(left.value, int)
        and left.value in (0, 1)
        and isinstance(right, extyper.types.Instance)
        and right.type.fullname == "builtins.bool"
    ):
        # Pretend Literal[0, 1] is a subtype of bool to avoid unhelpful errors.
        return True
    with extyper.state.strict_optional_set(True):
        return extyper.subtypes.is_subtype(left, right)


def get_mypy_type_of_runtime_value(runtime: Any) -> Optional[extyper.types.Type]:
    ...
    """
    if runtime is None:
        return extyper.types.NoneType()
    if isinstance(runtime, property):
        # Give up on properties to avoid issues with things that are typed as attributes.
        return None

    def anytype() -> extyper.types.AnyType:
        return extyper.types.AnyType(extyper.types.TypeOfAny.unannotated)
    ...
    if isinstance(
        ...
        fallback = extyper.types.Instance(type_info, [anytype()])
        signature = safe_inspect_signature(runtime)
        if signature:
            arg_types = []
            arg_kinds = []
        ...
        return extyper.types.CallableType(
            arg_types,
            arg_kinds,
            arg_names,
            ret_type=anytype(),
    ...
    if isinstance(runtime, tuple):
        ...
        fallback = extyper.types.Instance(type_info, [anytype()])
        return extyper.types.TupleType(items, fallback)

    fallback = extyper.types.Instance(type_info, [anytype() for _ in type_info.type_vars])
    try:
        ...
        return extyper.types.LiteralType(
            value=runtime,
            fallback=fallback,
        )
    except TypeError:
...
def build_stubs(modules: List[str], options: Options, find_submodules: bool = False) -> List[str]:
    ...
    """
    data_dir = extyper.build.default_data_dir()
    search_path = extyper.modulefinder.compute_search_paths([], options, data_dir)
    find_module_cache = extyper.modulefinder.FindModuleCache(
    ...
    for module in modules:
        ...
        if not find_submodules:
            ...
            if not isinstance(module_path, str):
                ...
                continue
            sources.append(extyper.modulefinder.BuildSource(module_path, module, None))
        else:
    ...
    try:
        res = extyper.build.build(sources=sources, options=options)
...
def get_typeshed_stdlib_modules(custom_typeshed_dir: Optional[str]) -> List[str]:
    ...
    stdlib_py_versions = extyper.modulefinder.load_stdlib_py_versions(custom_typeshed_dir)
    packages = set()
    ...
    if custom_typeshed_dir:
        ...
        typeshed_dir = Path(extyper.build.default_data_dir()) / "typeshed"
...
def main() -> int:
    extyper.util.check_python_version("stubtest")
