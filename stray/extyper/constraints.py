...
from extyper.types import is_generic

from extyper.types import (
    CallableType, MaybeTypes, Type, TypeVisitor, UnboundType, AnyType, NoneType, TypeVarType, Instance,
    TupleType, TypedDictType, UnionType, Overloaded, ErasedType, PartialType, DeletedType,
    UninhabitedType, TypeType, TypeVarId, TypeQuery, is_generic_instance, is_named_instance, TypeOfAny, LiteralType,
...
from extyper.nodes import COVARIANT, CONTRAVARIANT, ArgKind
from extyper.argmap import ArgTypeExpander
from extyper.typestate import TypeState
...
def infer_constraints_for_generic_arguments(
        callee: CallableType,
        generic_arg_types: Sequence[Optional[Type]],
    ...
    for i, actuals in enumerate(formal_to_actual):
        if not is_generic(callee.arg_types[i]):
            continue
        for actual in actuals:
            actual_arg_type = generic_arg_types[actual]
...
def infer_constraints_for_callable_pure(
        callee: CallableType,
...
def _infer_constraints(template: Type, actual: Type,
    ...
    if isinstance(template, UnionType):
        template = extyper.typeops.make_simplified_union(template.items, keep_erased=True)
    if isinstance(actual, UnionType):
        actual = extyper.typeops.make_simplified_union(actual.items, keep_erased=True)
...
def infer_constraints_if_possible(template: Type, actual: Type,
    ...
    if (direction == SUBTYPE_OF and
            not extyper.subtypes.is_subtype(erase_typevars(template), actual)):
        return None
    if (direction == SUPERTYPE_OF and
            not extyper.subtypes.is_subtype(actual, erase_typevars(template))):
        return None
    if (direction == SUPERTYPE_OF and isinstance(template, TypeVarType) and
            not extyper.subtypes.is_subtype(actual, erase_typevars(template.upper_bound))):
...
def is_same_constraint(c1: Constraint, c2: Constraint) -> bool:
    return (c1.type_var == c2.type_var
            and c1.op == c2.op
            and extyper.sametypes.is_same_type(c1.target, c2.target))
...
class ConstraintBuilderVisitor(TypeVisitor[List[Constraint]]):
    ...
    def visit_instance(self, template: Instance) -> List[Constraint]:
        ...
        if isinstance(actual, (CallableType, Overloaded)) and template.type.is_protocol:
            if template.type.protocol_members == ['__call__']:
                ...
                if not any(extyper.sametypes.is_same_type(template, t)
                           for t in template.type.inferring):
                    template.type.inferring.append(template)
                    call = extyper.subtypes.find_member('__call__', template, actual,
                                                     is_operator=True)
                    assert call is not None
                    if extyper.subtypes.is_subtype(actual, erase_typevars(call)):
        ...
        if isinstance(actual, Instance):
            ...
            if (template.type.is_protocol and self.direction == SUPERTYPE_OF and
                    ...
                    not any(extyper.sametypes.is_same_type(template, t)
                            for t in template.type.inferring) and
                    extyper.subtypes.is_protocol_implementation(instance, erased)):
                template.type.inferring.append(template)
                ...
                return res
                  ...
                  not any(extyper.sametypes.is_same_type(instance, i)
                          for i in instance.type.inferring) and
                  extyper.subtypes.is_protocol_implementation(erased, instance)):
        ...
        if (isinstance(actual, TupleType) and
            ...
            return infer_constraints(template,
                                     extyper.typeops.tuple_fallback(actual),
                                     self.direction)
        else:
            return []
    ...
    def infer_constraints_from_protocol_members(self, res: List[Constraint],
        ...
        for member in protocol.type.protocol_members:
            inst = extyper.subtypes.find_member(member, instance, subtype)
            temp = extyper.subtypes.find_member(member, template, subtype)
            assert inst is not None and temp is not None
            # The above is safe since at this point we know that 'instance' is a subtype
            # of (erased) 'template', therefore it defines all protocol members
            res.extend(infer_constraints(temp, inst, self.direction))
            if (extyper.subtypes.IS_SETTABLE in
                    extyper.subtypes.get_member_flags(member, protocol.type)):
    ...
    def visit_callable_type(self, template: CallableType) -> List[Constraint]:
        if isinstance(self.actual, CallableType):
            ...
            call = extyper.subtypes.find_member('__call__', self.actual, self.actual,
                                             is_operator=True)
            if call:
...
def find_matching_overload_item(overloaded: Overloaded, template: CallableType) -> CallableType:
    ...
    for item in items:
        ...
        # subtype check.
        if extyper.subtypes.is_callable_compatible(item, template,
                                                is_compat=extyper.subtypes.is_subtype,
                                                ignore_return=True):
            return item
