...
import extyper.applytype
import extyper.constraints
import extyper.typeops
import extyper.sametypes
from extyper.erasetype import erase_type
...
from extyper.expandtype import expand_type_by_instance
from extyper.typestate import TypeState, SubtypeKind
...
def check_type_parameter(lefta: Type, righta: Type, variance: int) -> bool:
    if variance == COVARIANT:
        ...
        return is_equivalent(lefta, righta) or is_subtype(lefta, righta)
...
def is_subtype(left: Type, right: Type,
    ...
    """
    if isinstance(left, (set, list)) or isinstance(right, (set, list)):
        # TODO: this call should not happen
        return True
    global incompatible_annot
    ...
    ret = _is_subtype(left, right,
                       ignore_type_params=ignore_type_params,
                       ignore_pos_arg_names=ignore_pos_arg_names,
                       ignore_declared_variance=ignore_declared_variance,
                       ignore_promotions=ignore_promotions, incompatible_annot=incompatible_annot)
    if not ret and incompatible_annot is not None:
        incompatible_annot.append((left, right))
    return ret
...
def _is_subtype(left: Type, right: Type,
                *,
                ...
                ignore_pos_arg_names: bool = False,
                ignore_declared_variance: bool = False,
                ignore_promotions: bool = False, incompatible_annot=None) -> bool:
    orig_right = right
    orig_left = left
    ...
    if (isinstance(right, AnyType) or isinstance(right, UnboundType)
        ...
        res = [is_subtype(orig_left, item,ignore_type_params=ignore_type_params,
                                            ignore_pos_arg_names=ignore_pos_arg_names,
                                            ignore_declared_variance=ignore_declared_variance,
                                            ignore_promotions=ignore_promotions)
                                 for item in right.items]
        is_subtype_of_item = any(res)
        ...
        if not is_subtype_of_item and isinstance(left, Instance) and left.type.is_enum:
            right = UnionType(extyper.typeops.try_contracting_literals_in_union(right.items))
            res = [is_subtype(orig_left, item,ignore_type_params=ignore_type_params,
                                            ignore_pos_arg_names=ignore_pos_arg_names,
                                            ignore_declared_variance=ignore_declared_variance,
                                            ignore_promotions=ignore_promotions)
...
class SubtypeVisitor(TypeVisitor[bool]):
    ...
    def visit_instance(self, left: Instance) -> bool:
        ...
        if isinstance(right, TupleType):#  and mypy.typeops.tuple_fallback(right).type.is_enum:
            return self._is_subtype(left, extyper.typeops.tuple_fallback(right))
        if isinstance(right, Instance):
            # if TypeState.is_cached_subtype_check(self._subtype_kind, left, right):
            #     return True
            if not self.ignore_promotions: # for int, float bool ... 
        ...
        if isinstance(right, TypeType):
            ...
            if isinstance(item, TupleType):
                item = extyper.typeops.tuple_fallback(item)
        ...
        if isinstance(right, PartialType) and right.type is None:
            return True
    ...
    def visit_type_var(self, left: TypeVarType) -> bool:
        ...
        if left.values and self._is_subtype(
                extyper.typeops.make_simplified_union(left.values), right):
            return True
    ...
    def visit_tuple_type(self, left: TupleType) -> bool:
        ...
        if isinstance(right, Instance):
            ...
            elif self._is_subtype(extyper.typeops.tuple_fallback(left), right):
                return True
            return False
            ...
            for l, r in zip(left.items, right.items):
                if not self._is_subtype(l, r):
                    return False
            rfallback = extyper.typeops.tuple_fallback(right)
            if is_named_instance(rfallback, 'builtins.tuple'):
                ...
                return True
            lfallback = extyper.typeops.tuple_fallback(left)
    ...
    def visit_union_type(self, left: UnionType) -> bool:
        ret = []
        for item in left.items:
            res = self._is_subtype(item, self.orig_right)
            ret.append(res)
        return all(ret)
...
class SubtypePowerVisitor(TypeVisitor[bool]):
    ...
    def visit_instance(self, left: Instance) -> bool:
        if left.type.fallback_to_any:
            ...
            return True
        right = self.right
        if isinstance(right, TupleType) and extyper.typeops.tuple_fallback(right).type.is_enum:
            return self._is_subtype(left, extyper.typeops.tuple_fallback(right))
        if isinstance(right, Instance):
            if TypeState.is_cached_subtype_check(self._subtype_kind, left, right):
                return True
...
def find_member(name: str,
    ...
    if method:
        if method.is_property and isinstance(method, OverloadedFuncDef):
            dec = method.items[0]
...
def find_node_type(node: Union[Var, FuncBase], itype: Instance, subtype: Type) -> Type:
    ...
    """
    from extyper.typeops import bind_self

    if isinstance(node, FuncBase):
        typ: Optional[Type] = extyper.typeops.function_type(
...
def is_callable_compatible(left: CallableType, right: CallableType,
    ...
    for right_arg in right.formal_arguments():
        left_arg = extyper.typeops.callable_corresponding_argument(left, right_arg)
        if left_arg is None:
...
def unify_generic_callable(type: CallableType, target: CallableType,
    ...
    """
    import extyper.solve

    if return_constraint_direction is None:
        return_constraint_direction = extyper.constraints.SUBTYPE_OF

    constraints: List[extyper.constraints.Constraint] = []
    for arg_type, target_arg_type in zip(type.arg_types, target.arg_types):
        c = extyper.constraints.infer_constraints(
            arg_type, target_arg_type, extyper.constraints.SUPERTYPE_OF)
    ...
    inferred_vars = extyper.solve.solve_constraints(type_var_ids, constraints)
    ...
    applied = extyper.applytype.apply_generic_arguments(type, non_none_inferred_vars, report,
                                                     context=target)
    if had_errors:
        return None
    return applied
...
class ProperSubtypeVisitor(TypeVisitor[bool]):
            ...
            if left.type.has_base(right.type.fullname):
                def check_argument(leftarg: Type, rightarg: Type, variance: int) -> bool:
                    if variance == COVARIANT:
                        ...
                        return extyper.sametypes.is_same_type(leftarg, rightarg)
                # Map left type to corresponding right instances.
    ...
    def visit_type_var(self, left: TypeVarType) -> bool:
        ...
        if left.values and self._is_proper_subtype(
                extyper.typeops.make_simplified_union(left.values), self.right):
            return True
    ...
    def visit_tuple_type(self, left: TupleType) -> bool:
        ...
        if isinstance(right, Instance):
            ...
            return self._is_proper_subtype(extyper.typeops.tuple_fallback(left), right)
        elif isinstance(right, TupleType):
            if len(left.items) != len(right.items):
                return False
            for l, r in zip(left.items, right.items):
                if not self._is_proper_subtype(l, r):
                    return False
            return self._is_proper_subtype(extyper.typeops.tuple_fallback(left),
                                           extyper.typeops.tuple_fallback(right))
        return False
        ...
        if isinstance(right, TypedDictType):
            for name, typ in left.items.items():
                if (name in right.items
                        and not extyper.sametypes.is_same_type(typ, right.items[name])):
                    return False
