...
from extyper.join import join_simple
from extyper.sametypes import is_same_type
from extyper.erasetype import remove_instance_last_known_values
from extyper.nodes import Expression, FlowNode, TempNode, Var, RefExpr
from extyper.literals import Key, literal, literal_hash, subkeys
from extyper.nodes import IndexExpr, MemberExpr, NameExpr
...
class ConditionalTypeBinder:
    ...
    def _put2(self, key: Expression, type: Type, index: int = -1) -> None:
        self.frames[index].record = (key, type)
    ...
    def pop_frame(self, can_skip: bool, fall_through: int) -> Frame:
        ...
        # len(options) may > 1, e.g., if:xxx\ else:xxx\
        if can_skip:
    ...
    def allow_jump(self, index: int) -> None:
        ...
        fall_type = AnyType(0)
        fall_node = FlowNode(fall_type)
        fall_pairs = []
        for f in self.frames[index + 1:]:
            frame.types.update(f.types)
            if f.record is not None:
                fall_pairs.append(f.record)
            if f.unreachable:
...
def get_declaration(expr: BindableExpression, var_node=None, type_map=None) -> Optional[Type]:
    if isinstance(expr, RefExpr) and isinstance(expr.node, Var):
        ...
        if type is not None:
            ...
            if node in var_node:
                defs = var_node[node]
                if defs[0] in type_map:
                    return type_map[defs[0]]
