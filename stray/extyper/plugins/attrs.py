...
import extyper.plugin  # To avoid circular imports.
from extyper.exprtotype import expr_to_unanalyzed_type, TypeTranslationError
from extyper.fixup import lookup_qualified_stnode
...
from extyper.plugin import SemanticAnalyzerPluginInterface
from extyper.plugins.common import (
...
class Attribute:
    ...
    def argument(self, ctx: 'extyper.plugin.ClassDefContext') -> Argument:
        ...
        if self.converter.name:
            ...
            if converter and isinstance(converter.node, TypeInfo):
                from extyper.checkmember import type_object_type  # To avoid import cycle.
...
def _determine_eq_order(ctx: 'extyper.plugin.ClassDefContext') -> bool:
    """
...
def _get_decorator_optional_bool_argument(
    ctx: 'extyper.plugin.ClassDefContext',
    name: str,
...
def attr_class_maker_callback(ctx: 'extyper.plugin.ClassDefContext',
...
def _get_frozen(ctx: 'extyper.plugin.ClassDefContext', frozen_default: bool) -> bool:
...
def _analyze_class(ctx: 'extyper.plugin.ClassDefContext',
...
def _detect_auto_attribs(ctx: 'extyper.plugin.ClassDefContext') -> bool:
...
def _attributes_from_assignment(ctx: 'extyper.plugin.ClassDefContext',
...
def _attribute_from_auto_attrib(ctx: 'extyper.plugin.ClassDefContext',
                                kw_only: bool,
                                lhs: NameExpr,
                                rvalue: Expression,
...
def _attribute_from_attrib_maker(ctx: 'extyper.plugin.ClassDefContext',
                                 auto_attribs: bool,
                                 kw_only: bool,
                                 lhs: NameExpr,
                                 rvalue: CallExpr,
...
def _parse_converter(ctx: 'extyper.plugin.ClassDefContext',
...
def _add_order(ctx: 'extyper.plugin.ClassDefContext', adder: 'MethodAdder') -> None:
...
def _make_frozen(ctx: 'extyper.plugin.ClassDefContext', attributes: List[Attribute]) -> None:
...
def _add_init(ctx: 'extyper.plugin.ClassDefContext', attributes: List[Attribute],
...
def _add_attrs_magic_attribute(ctx: 'extyper.plugin.ClassDefContext',
...
class MethodAdder:
    ...
    def __init__(self, ctx: 'extyper.plugin.ClassDefContext') -> None:
        self.ctx = ctx
