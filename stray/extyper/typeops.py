...
from extyper.expandtype import expand_type_by_instance, expand_type
...
def bind_self(method: F, original_type: Optional[Type] = None, is_classmethod: bool = False) -> F:
    ...
    """
    from extyper.infer import infer_type_arguments
...
def callable_corresponding_argument(typ: CallableType,
    ...
    if by_name is not None and by_pos is not None:
        ...
        from extyper.subtypes import is_equivalent
...
def make_simplified_union(items: Sequence[Type],
    ...
    * [Any, Any] -> Any
    ...
    from extyper.subtypes import is_proper_subtype
...
def true_only(t: Type) -> ProperType:
    ...
    t = get_proper_type(t)
    judge = t
    if isinstance(t, Instance) and t.last_known_value is not None and isinstance(t.last_known_value, LiteralType):
        judge = t.last_known_value

    if not judge.can_be_true:
        # All values of t are False-ish, so there are no true values in it
        return UninhabitedType(line=t.line, column=t.column)
    elif not judge.can_be_false:
