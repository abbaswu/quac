...
from extyper.backports import OrderedDict
...
import extyper.strconv
from extyper.util import short_type
from extyper.visitor import NodeVisitor, StatementVisitor, ExpressionVisitor

from extyper.bogus_type import Bogus
...
class Node(Context):
    ...
    __slots__ = ('temp_method1', 'temp_return1', 'temp_method2', 'temp_return2', 'depth', 'callee')

    def __str__(self) -> str:
        ans = self.accept(extyper.strconv.StrConv())
        if ans is None:
            return repr(self)
        return ans
    ...
    def set_depth(self, depth):
        self.depth = depth
    def get_method(self):
        return getattr(self, f'temp_method{self.depth}')
    def get_return(self):
        return getattr(self, f'temp_return{self.depth}')
    
    def set_method(self, temp_method):
        if not hasattr(self, 'depth'):
            self.temp_method1 = temp_method
        else:
            setattr(self,f'temp_method{self.depth}', temp_method)

    def set_return(self, temp_return):
        if not hasattr(self, 'depth'):
            self.temp_return1 = temp_return
        else:
            setattr(self,f'temp_return{self.depth}', temp_return)
...
class FuncBase(Node):
    ...
    def __init__(self) -> None:
        ...
        self.type: Optional[extyper.types.ProperType] = None
        # Original, not semantically analyzed type (used for reprocessing)
        self.unanalyzed_type: Optional[extyper.types.ProperType] = None
...
class OverloadedFuncDef(FuncBase, SymbolNode, Statement):
    ...
    def deserialize(cls, data: JsonDict) -> 'OverloadedFuncDef':
        ...
        if data.get('type') is not None:
            typ = extyper.types.deserialize_type(data['type'])
            assert isinstance(typ, extyper.types.ProperType)
            res.type = typ
...
class Argument(Node):
    ...
    def __init__(self,
                 variable: 'Var',
                 type_annotation: 'Optional[extyper.types.Type]',
    ...
    def accept(self, visitor: ExpressionVisitor[T]) -> T:
        return visitor.visit_argument_expr(self)
...
class FuncItem(FuncBase):
    ...
    def __init__(self,
                 ...
                 body: 'Block',
                 typ: 'Optional[extyper.types.FunctionLike]' = None) -> None:
        super().__init__()
    ...
    def deserialize(cls, data: JsonDict) -> 'FuncDef':
        ...
        ret = FuncDef(data['name'],
                      ...
                      (None if data['type'] is None
                       else cast(extyper.types.FunctionLike,
                                 extyper.types.deserialize_type(data['type']))))
...
class Decorator(SymbolNode, Statement):
    ...
    @property
    def type(self) -> 'Optional[extyper.types.Type]':
...
class Var(SymbolNode):
    ...
    def __init__(self, name: str, type: 'Optional[extyper.types.Type]' = None) -> None:
        super().__init__()
        ...
        self.type: Optional[extyper.types.Type] = type  # Declared or inferred type, or None
    ...
    def deserialize(cls, data: JsonDict) -> 'Var':
        ...
        name = data['name']
        type = None if data['type'] is None else extyper.types.deserialize_type(data['type'])
        v = Var(name, type)
...
class ClassDef(Statement):
    ...
    defs: "Block"
    type_vars: List["extyper.types.TypeVarType"]
    ...
    def __init__(self,
                 name: str,
                 defs: 'Block',
                 type_vars: Optional[List['extyper.types.TypeVarType']] = None,
    ...
    def deserialize(self, data: JsonDict) -> 'ClassDef':
        ...
        res = ClassDef(data['name'],
                       Block([]),
                       [extyper.types.TypeVarType.deserialize(v) for v in data['type_vars']],
                       )
...
class AssignmentStmt(Statement):
    ...
    type: Optional["extyper.types.Type"] = None
    # Original, not semantically analyzed type in annotation (used for reprocessing)
    unanalyzed_type: Optional["extyper.types.Type"] = None
    ...
    def __init__(self, lvalues: List[Lvalue], rvalue: Expression,
                 type: 'Optional[extyper.types.Type]' = None, new_syntax: bool = False) -> None:
        super().__init__()
...
class ForStmt(Statement):
    ...
    index_type: Optional["extyper.types.Type"] = None
    # Original, not semantically analyzed type in annotation (used for reprocessing)
    unanalyzed_index_type: Optional["extyper.types.Type"] = None
    # Inferred iterable item type
    inferred_item_type: Optional["extyper.types.Type"] = None
    # Inferred iterator type
    inferred_iterator_type: Optional["extyper.types.Type"] = None
    ...
    def __init__(self,
                 ...
                 index_type: 'Optional[extyper.types.Type]' = None) -> None:
        super().__init__()
        self.index = index
...
class RefExpr(Expression):
    ...
    def __init__(self) -> None:
        ...
        self.type_guard: Optional["extyper.types.Type"] = None
...
class ComparisonExpr(Expression):
    ...
    method_types: List[Optional["extyper.types.Type"]]
...
class CastExpr(Expression):
    ...
    expr: Expression
    type: "extyper.types.Type"

    def __init__(self, expr: Expression, typ: 'extyper.types.Type') -> None:
        super().__init__()
        self.expr = expr
        self.type = typ
...
class TypeApplication(Expression):
    ...
    expr: Expression
    types: List["extyper.types.Type"]

    def __init__(self, expr: Expression, types: List['extyper.types.Type']) -> None:
        super().__init__()
        self.expr = expr
        self.types = types
...
class TypeVarLikeExpr(SymbolNode, Expression):
    ...
    upper_bound: "extyper.types.Type"
    ...
    def __init__(
        self, name: str, fullname: str, upper_bound: 'extyper.types.Type', variance: int = INVARIANT
    ) -> None:
        super().__init__()
        self._name = name
...
class TypeVarExpr(TypeVarLikeExpr):
    ...
    values: List["extyper.types.Type"]
    ...
    def deserialize(cls, data: JsonDict) -> 'TypeVarExpr':
        ...
        return TypeVarExpr(data['name'],
                           data['fullname'],
                           [extyper.types.deserialize_type(v) for v in data['values']],
                           extyper.types.deserialize_type(data['upper_bound']),
                           data['variance'])
...
class PromoteExpr(Expression):
    ...
    def __init__(self, type: 'extyper.types.Type') -> None:
        super().__init__()
        self.type = type
...
class NewTypeExpr(Expression):
    ...
    old_type: Optional["extyper.types.Type"] = None
    # The synthesized class representing the new type (inherits old_type)
    info: Optional["TypeInfo"] = None

    def __init__(self, name: str, old_type: 'Optional[extyper.types.Type]', line: int,
                 column: int) -> None:
        super().__init__()
        self.name = name
...
# Constants
class FlowNode(Expression):
    """Temporary flow node used during type checking.

    This node is not present in the original program; it is just an artifact
    of the type checker implementation. It only represents an control flow node.
    """
    __slot__ = ('context')
    type: "extyper.types.Type"
    ...
    def __repr__(self) -> str:
        return 'FlowNode:%d(%s)' % (self.line, str(self.type))
...
class TempNode(Expression):
    ...
    def __init__(self,
                 ...
                 no_rhs: bool = False,
                 *,
                 context: Optional[Context] = None, desc:str = None) -> None:
        """Construct a dummy node; optionally borrow line/column from context object."""
        super().__init__()
        self.type = typ
        self.no_rhs = no_rhs
...
class TypeInfo(SymbolNode):
    ...
    declared_metaclass: Optional["extyper.types.Instance"] = None
    metaclass_type: Optional["extyper.types.Instance"] = None
    ...
    assuming: List[Tuple["extyper.types.Instance", "extyper.types.Instance"]]
    assuming_proper: List[Tuple["extyper.types.Instance", "extyper.types.Instance"]]
    ...
    # -> infer_constraints.
    inferring: List["extyper.types.Instance"]
    ...
    # Direct base classes.
    bases: List["extyper.types.Instance"]
    ...
    _promote: Optional["extyper.types.Type"] = None
    ...
    # tuple type.
    tuple_type: Optional["extyper.types.TupleType"] = None
    ...
    typeddict_type: Optional["extyper.types.TypedDictType"] = None
    ...
    def get_method(self, name: str) -> Optional[FuncBase]:
        ...
        #         if isinstance(node, FuncBase) and node.type is not None:
        #             return node
        #         # else:
        #         #     return None
        for cls in self.mro:
    ...
    def calculate_metaclass_type(self) -> 'Optional[extyper.types.Instance]':
        declared = self.declared_metaclass
        if declared is not None and not declared.type.has_base('builtins.type'):
            return declared
        if self._fullname == 'builtins.type':
            return extyper.types.Instance(self, [])
    ...
    def dump(self,
             str_conv: 'Optional[extyper.strconv.StrConv]' = None,
             type_str_conv: 'Optional[extyper.types.TypeStrVisitor]' = None) -> str:
        """Return a string dump of the contents of the TypeInfo."""
        if not str_conv:
            str_conv = extyper.strconv.StrConv()
        base: str = ""

        def type_str(typ: 'extyper.types.Type') -> str:
            if type_str_conv:
        ...
        return extyper.strconv.dump_tagged(
            items,
            head,
            str_conv=str_conv)
    ...
    def deserialize(cls, data: JsonDict) -> 'TypeInfo':
        ...
        ti.bases = [extyper.types.Instance.deserialize(b) for b in data['bases']]
        ti._promote = (None if data['_promote'] is None
                       else extyper.types.deserialize_type(data['_promote']))
        ti.declared_metaclass = (None if data['declared_metaclass'] is None
                                 else extyper.types.Instance.deserialize(data['declared_metaclass']))
        ti.metaclass_type = (None if data['metaclass_type'] is None
                             else extyper.types.Instance.deserialize(data['metaclass_type']))
        ...
        ti.tuple_type = (None if data['tuple_type'] is None
                         else extyper.types.TupleType.deserialize(data['tuple_type']))
        ti.typeddict_type = (None if data['typeddict_type'] is None
                            else extyper.types.TypedDictType.deserialize(data['typeddict_type']))
...
class TypeAlias(SymbolNode):
    ...
    def __init__(self, target: 'extyper.types.Type', fullname: str, line: int, column: int,
                 *,
    ...
    def deserialize(cls, data: JsonDict) -> 'TypeAlias':
        ...
        target = extyper.types.deserialize_type(data['target'])
