...
from extyper.typetraverser import TypeTraverserVisitor
from extyper.nodes import (
    MemberExpr, OpExpr, TypeInfo, Context, MypyFile, FuncDef, reverse_builtin_aliases,
    ArgKind, ARG_POS, ARG_OPT, ARG_NAMED, ARG_NAMED_OPT, ARG_STAR, ARG_STAR2,
    ReturnStmt, NameExpr, Var, CONTRAVARIANT, COVARIANT, SymbolNode,
    CallExpr, IndexExpr, StrExpr, SymbolTable, TempNode, SYMBOL_FUNCBASE_TYPES, NameExpr
)
from extyper.operators import op_methods, op_methods_to_symbols
...
from extyper import message_registry, errorcodes as codes
...
class MessageBuilder:
    ...
    def has_no_attr(self,
        ...
        member_expr = None
        # if isinstance(context, MemberExpr):
        #     member_expr = context
        # elif isinstance(context, CallExpr):
        #     member_expr = context.callee
        # elif isinstance(context, OpExpr):
        #     member_expr = context.left
        # if member_expr:
        #     if isinstance(member_expr,  MemberExpr):
        #         identity = get_identity(self.errors.cscope.top_function(), member_expr.expr.node)
        #         if identity is not None:
        #             if identity not in self.errors.single_incompatible:
        #                 self.errors.single_incompatible[identity] = []
        #             self.errors.single_incompatible[identity].append(typ)
        #     elif isinstance(member_expr, NameExpr):
        #         identity = get_identity(self.errors.cscope.top_function(), member_expr.node)
        #         if identity is not None:
        #             if identity not in self.errors.single_incompatible:
        #                 self.errors.single_incompatible[identity] = []
        #             self.errors.single_incompatible[identity].append(typ)
    ...
    def unsupported_left_operand(self, op: str, typ: Type,
                                 context: Context) -> None: # merge: has no attr
        if self.disable_type_names:
    ...
    def invalid_index_type(self, index_type: Type, expected_type: Type, base_str: str,
                           context: Context, *, code: ErrorCode) -> None: # merge: incompatible argument
    ...
    def no_variant_matches_arguments(self,
                                     ...
                                     context: Context,
                                     *,
                                     code: Optional[ErrorCode] = None) -> None: # operator: op
    ...
    def wrong_number_values_to_unpack(self, provided: int, expected: int,
                                      context: Context) -> None: #op: assignment
    ...
    def unpacking_strings_disallowed(self, context: Context) -> None: # op: assignment 
        self.fail("Unpacking a string is disallowed", context)

    def type_not_iterable(self, type: Type, context: Context) -> None: # op: assignment 
    ...
    def overload_signature_incompatible_with_supertype(
            ...
            overload: Overloaded, context: Context) -> None: # op: override
    ...
    def signature_incompatible_with_supertype(
            ...
            override: Optional[FunctionLike] = None) -> None: # op: override
        code = codes.OVERRIDE
        target = self.override_target(name, name_in_super, supertype)
        self.fail('Signature of "{}" incompatible with {}'.format(
    ...
    def pretty_callable_or_overload(self,
                                    ...
                                    *,
                                    ...
                                    allow_dups: bool = False,
                                    code: Optional[ErrorCode] = None) -> None:# op: override
    ...
    def incompatible_type_application(self, expected_arg_count: int,
                                      ...
                                      context: Context) -> None: # operator: apply
