...
from extyper.nodes import (
    Expression, Var, ComparisonExpr, OpExpr, MemberExpr, UnaryExpr, StarExpr, IndexExpr, LITERAL_YES,
    ...
    TypedDictExpr, NewTypeExpr, PromoteExpr, AwaitExpr, TempNode, AssignmentExpr, ParamSpecExpr, Argument
)
...
class _Hasher(ExpressionVisitor[Optional[Key]]):
    ...
    def visit_argument_expr(self, e: Argument) -> Key:
        # N.B: We use the node itself as the key, and not the name,
        # because using the name causes issues when there is shadowing
        # (for example, in list comprehensions).

        # return e.variable.name
        return ('Var', e.variable)
